...
class Subgraph():
    def __init__(self, network: Literal['mainnet', 'gnosis']) -> None:
...
class KlerosBoardSubgraph(Subgraph):
    def __init__(self, network: Literal['mainnet', 'gnosis']='mainnet') -> None:
        super(KlerosBoardSubgraph, self).__init__(network=network)
        self.logger: logging.Logger = logging.getLogger(__name__)
    ...
    @staticmethod
    def _calculateVoteStake(minStake, alpha) -> float:
        return float(alpha) * (10 ** -4) * float(minStake)

    def _getBlockNumberbefore(self, days=30) -> int:
        """
    ...
    @staticmethod
    def _getHistoricPrices(timestamp_from, network='mainnet') -> pd.DataFrame:
        now = datetime.now()
    ...
    def _parseDraw(self, draw):
        ...
        if 'timestamp' in keys:
            try:
                draw['timestamp'] = int(draw['timestamp'])
            except TypeError:
                draw['timestamp'] = None
        if 'disputeId' in keys:
            draw['disputeId'] = int(draw['disputeId'])
        if 'roundNumber' in keys:
            draw['roundNumber'] = int(draw['roundNumber'])
        if 'voteId' in keys:
            draw['voteId'] = int(draw['voteId'])
        return draw
    ...
    def getAllDraws(self):
        ...
        while True:
            query = ('{draws('
                    ...
                    f'timestamp_gt:{initTimestamp}'
                    '}){'
                    'id,timestamp,address,disputeId,roundNumber,voteId'
                    '}}'
                    )
            result = self._post_query(query)
            if result is None:
    ...
    def getAllTransfers(self):
        ...
        while True:
            query = (
                '{tokenAndETHShifts(where:{timestamp_gt:"' + str(initTimestamp) + '"'
                ',ETHAmount_gt:0},'
            ...
            if result is None:
                ...
                if len(currenttransfers) < 1000:
                    break
                initTimestamp = currenttransfers[-1]['timestamp']
    ...
    def getAllVotes(self):
        ...
        while True:
            query = ('{votes('
                    ...
                    'choice,voted,round{id},timestamp'
                    '}}'
                    )
            result = self._post_query(query)
            if result is None:
            ...
            initTimestamp = result['votes'][-1]['timestamp']
    ...
    def getAllJurors(self):
        ...
        while True:
            query = (
                ...
                'orderDirection:asc){id,totalStaked,numberOfDisputesAsJuror,ethRewards'
                '}}'
            )
            result = self._post_query(query)
            if result is None:
    ...
    def getAllTransactions(self) -> pd.DataFrame:
        votes = self.getAllVotes()
        disputes = self.getAllDisputes()
        stakes = self.getAllStakeSets()
        transfers = self.getAllTransfers()
        draws = self.getAllDraws()

        df_stakes = pd.DataFrame(stakes)
        df_stakes['tx'] = 'SetStake'
        df_stakes['timestamp'] = pd.to_datetime(df_stakes['timestamp'], unit='s')

        df_disputes = pd.DataFrame(disputes)
        df_disputes['tx'] = 'NewDispute'
        df_disputes['timestamp'] = pd.to_datetime(df_disputes['startTime'], unit='s')

        df_votes = pd.DataFrame(votes)
        df_votes['tx'] = 'Vote'
        df_votes['timestamp'] = pd.to_datetime(df_votes['timestamp'], unit='s')

        df_transfers = pd.DataFrame(transfers)
        df_transfers['tx'] = 'Transfer'
        df_transfers['timestamp'] = pd.to_datetime(df_transfers['timestamp'], unit='s')

        df_draws = pd.DataFrame(draws)
        df_draws['tx'] = 'Draw'
        df_draws['timestamp'] = pd.to_datetime(df_draws['timestamp'], unit='s')
